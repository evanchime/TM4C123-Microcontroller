...
// Subroutine to set PF1(output) low, if Switches not pressed
// input: none
// output : none
// notes: ..............
void Clear_Output(void){
	GPIO_PORTF_DATA_R |= 0x11; // make sure PF1(output) is low
}
...
int main(void){  unsigned long i,last,now;
  ...
  while(1){unsigned long SW1,SW2;
		SW1 = GPIO_PORTF_DATA_R & 0x10; // read PF4(Switch 1)
		SW2 = GPIO_PORTF_DATA_R & 0x01; // read PF0(Switch 2)
    if(!(SW1) || !(SW2)){
