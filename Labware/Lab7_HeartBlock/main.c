...
void WaitForASLow(void){
  unsigned long SW1;
  do{
	  SW1 = GPIO_PORTF_DATA_R; // read input PF4
	}while( SW1 & 0x10 ); // check if AS low ie SW1 pressed
}
...
void WaitForASHigh(void){
    ...
    do{
			SW1 = GPIO_PORTF_DATA_R; // read input PF4
		}while( !(SW1 & 0x10) ); // check if AS high ie SW1 released
}
...
void SetVT(void){
    GPIO_PORTF_DATA_R  |= 0x02; // write this function
}
...
// Notes:   friendly means it does not affect other bits in the port
void SetReady(void){
    GPIO_PORTF_DATA_R |= 0x08;// write this function
}
